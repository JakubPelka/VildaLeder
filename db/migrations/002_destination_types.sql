...
ALTER TYPE spatial_feature_kind RENAME TO spatial_feature_kind_v1;
...
DROP TYPE spatial_feature_kind_v1;
...
ADD CONSTRAINT spatial_feature_kind_geometry_check CHECK (
    ...
    OR (
        feature_kind IN ('reserve', 'national_park')
        AND GeometryType(geom) IN ('POLYGON', 'MULTIPOLYGON')
    )
    OR (
        feature_kind IN ('bird_hide', 'observation_tower', 'observation_site')
...
VALUES (2, '002_destination_types.sql');
